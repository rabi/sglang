...
if _is_hip:
    if _use_aiter:
        ...
        def _dynamic_per_token_scaled_fp8_quant_pytorch(
            ...
            max_vals = torch.abs(input_contig).max(dim=1, keepdim=True)[0]  # [num_tokens, 1]
            # Calculate scale per token: scale = max_value / fp8_max
            # Avoid division by zero
            max_vals = torch.clamp(max_vals, min=1e-10)
            scale.copy_((max_vals / fp8_max).to(scale.dtype))
            ...
            output.copy_(quantized_float.to(output.dtype))
        ...
        if scale is None:
            ...
            if use_per_token_if_dynamic:
                ...
                if _use_aiter:
                    ...
                    _dynamic_per_token_scaled_fp8_quant_pytorch(
                        output, input, scale
                    )
            else:
